...
class SFB_Validator
{
	...
	public function walk_fields( &$form_fields, $values )
	{
		...
		foreach ( $form_fields as $field_name => $field_args )
		{
			 ...
			 * return mixed
			 */
			$field_value = apply_filters( 'wp_sfb_field_value', $field_value, $field_name, $field_args, $this->form_id );
		}

		/**
		 * Filter fields validation results
		 *
		 * @param array|WP_Error $form_values
		 * @param array          $form_fields
		 * @param string         $form_id
		 *
	...
	{
		$slider_options = wp_parse_args( $field_args['slider_options'], [
			'range' => false,
			'min'   => 0,
			'max'   => 100,
		] );
	...
	{
		$editor_settings = wp_parse_args( $field_args['data_type_options'], [
			'allowed_html'      => array_merge( wp_kses_allowed_html(), [
				'h1'  => [],
				'h3'  => [],
				'h2'  => [],
				'p'   => [],
	...
	{
		$type_options = wp_parse_args( $field_args['data_type_options'], [
			'format' => 'Y-m-d',
		] );
	...
	public function data_type_number( $field_value, $field_name, $field_args )
	{
		...
		else
		{
			...
			if ( $type_options['max'] !== null && false === filter_var( $field_value, FILTER_VALIDATE_INT, [ 'options' => [ 'max_range' => $type_options['max'] ] ] ) )
			{
				return new WP_Error( 'number-range', sprintf( __( '%s maximum is %s.' ), $field_args['label'], $type_options['max'] ) );
			}
		}
